...
def check_for_draw(game_board):
  ...
  if turn_count == 9 and not check_for_win(game_board):
    return True
  else:
    return False       

def mini_max(game_board,max):
    copy_board = game_board.copy()
    positions_available = [int(x) - 1 for x in copy_board if x not in ['X','O']]
    for mm_pos in positions_available:
      if check_for_win(game_board):
        return 10 if max else -10
      elif check_for_draw(game_board):
        return 0  
      else:
        rev_max = not max
        mini_max(game_board,rev_max)
